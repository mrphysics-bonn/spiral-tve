...
def gre_refscan_B0(seq, prot=None, system=Opts(), params=None, grads_off=False):
    ...
    system.max_slew = min(170*system.gamma, save_slew)
    system.max_grad = min(50e-3*system.gamma, save_grad)
    ...
    if params["separate_tr"]:
        d_TE = np.asarray(params["TE"][2:]) - np.asarray(params["TE"][:-2])
        delay_TE.append(params["TE"][1] - min_TE)
    if delay_TE[0] < 0:
    ...
    # ADC 
    adc = make_adc(num_samples=samples, dwell=dwelltime, delay=gx.rise_time+round(diff_flat_adc/2, 6), system=system)
    ...
    # build sequence
    if params['separate_tr']:
        TR = slices * (rf.delay + params["rf_dur"] / 2 + params["TE"][-1]
             + calc_duration(gx)/2 + calc_duration(gx_spoil,max_gy_pre,max_gz_rew))
    else:
    ...
    if not grads_off:
        ...
        print(f"Ernst Angles for\n"
            f"T1 = 1000 ms: {np.rad2deg(np.arccos(np.exp(-TR/1))):.2f} deg\n"
            f"T1 = 1500 ms: {np.rad2deg(np.arccos(np.exp(-TR/1.5))):.2f} deg\n"
            f"T1 = 2000 ms: {np.rad2deg(np.arccos(np.exp(-TR/2))):.2f} deg")
   ...
   # turn off gradients, if selected
    ...
    if not params['separate_tr']:
        ...
        for i in range(-prepscans, Ny):
            ...
            # RF spoiling
            spoil_phase = rf_phase / 180 * np.pi
            ...
            for s in range(slices):
                ...
                rf.phase_offset = spoil_phase - 2 * np.pi * rf.freq_offset * calc_rf_center(rf)[0]
        ...
        delays_list = [delay_TE[::2], delay_TE[1::2]]
        ...
        for i in range(-prepscans, Ny):
            ...
            for d, delays in enumerate(delays_list):
                if slices%2 == 1:
                    slc = 0
                else:
                    slc = 1
                ...
                for s in range(slices):
                    ...
                    for nz in range(Nz):
                        ...
                        for j,delay in enumerate(delays):
                            if j==0:
                                delay_pre = make_delay(delay+pre_dur)
                                seq.add_block(gx_pre, gy_pre, gz_pre, delay_pre)
                            else:
                            ...
                            if j < len(delays)-1:
                                seq.add_block(gx_mid)
                        if (n_TE%2 == 0 and d==0) or (n_TE%2 == 1 and d==1):
                            seq.add_block(make_delay(d=params["TE"][-1]-params["TE"][-2]))
                        gy_pre.amplitude = -gy_pre.amplitude
                        gy_pre.delay = 0
