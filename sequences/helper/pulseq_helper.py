...
def round_up_to_raster(number, decimals=5, trunc_first=True):
    """
    round number up to a specific number of decimal places.
    If trunc_first is true, the number is first truncated to "decimals+1" digits.
    In this way, only the decimal after "decimals" is taken into account for rounding.
    This avoids round up numbers as 2.3000000001 up to 2.4 (if decimals=1)
    """

    if trunc_first:
        number = trunc_to_raster(number, decimals+1)
