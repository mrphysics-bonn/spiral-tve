function [c,ceq] = nlcon(x, b_val_max, refoc_dur, includePTE, maxgrad, maxslew)
    ...
    x_ramp = round((maxgrad * gamma)/((maxslew*0.94) * gamma), 5);
    ...
    if includePTE
        c(1) = Gx / ((0.97*maxgrad)/sqrt(3/2) * gamma) - 1.0;
        c(2) = Gy / ((0.97*maxgrad)/sqrt(3/2) * gamma) - 1.0;
        c(3) = Gz / ((0.97*maxgrad)/sqrt(3/2) * gamma) - 1.0;
        c(4) = x(1)*alpha_z / (maxgrad/sqrt(3/2) * gamma) - 1.0;
    else
        c(1) = Gx / ((0.97*maxgrad) * gamma) - 1.0;
        c(2) = Gy / ((0.97*maxgrad) * gamma) - 1.0;
        c(3) = Gz / ((0.97*maxgrad) * gamma) - 1.0;
        c(4) = x(1)*alpha_z / (maxgrad * gamma) - 1.0;
    end
    c(5) = -1 * Gx;
    c(6) = -1 * Gy;
    c(7) = -1 * Gz;
